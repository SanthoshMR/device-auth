...
module.exports = {
   ...
   * @api public
   */
  createJwt: function(req, res, device) {
    var jsonWebTokens = waterlock.config.deviceJsonWebTokens || {};
    ...
    var issued = Date.now();
    device = device || req.session.device;

    var token = jwt.encode({
      iss: device.id + '|' + req.remoteAddress,
